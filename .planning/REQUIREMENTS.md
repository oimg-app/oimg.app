...
- [ ] **PIPE-01**: Optimization runs off the main thread via a Comlink-wrapped WorkerPool — the UI stays responsive (no jank) while files encode
- [ ] **PIPE-02**: Codecs are dynamically imported inside workers so the initial route stays < 200KB gzipped; AVIF loads only when the user selects it
- [ ] **PIPE-03**: COOP/COEP response headers are configured so SharedArrayBuffer is available for multi-threaded codecs (OxiPNG, AVIF)
...
- [ ] **ENC-01**: User can optimize a PNG via OxiPNG (decode through @jsquash/png, re-encode) and get real reduced-size output
- [ ] **ENC-02**: User can encode/convert to WebP with quality and lossless controls
